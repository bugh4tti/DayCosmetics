...
public class KillEffectsMenu {
    ...
    public static void open(Player player) {
        ...
        Inventory inv = Bukkit.createInventory(null, 54,
                plugin.getConfig().getString("menus.killeffects.title"));

        MenuUtils.fillBorders(inv);

        ConfigurationSection sec = plugin.getConfig().getConfigurationSection("killeffects");
        for (String key : sec.getKeys(false)) {
            String path = "killeffects." + key + ".";
